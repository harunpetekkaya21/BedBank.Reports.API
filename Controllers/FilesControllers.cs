...
namespace BedBankReports.API.Controllers
{
    ...
    public class FilesController : ControllerBase
    {
        ...
        public FilesController(IFileRepository fileRepository, IHotelRepository hotelRepository, IRateRepository rateRepository)
        {
        ...
        private async Task ReadExcelFileAndSave(IFormFile myFile, string filePath)
        {
            ...
            foreach (var item in rateListFromDataTable)
            {
                ...
                CreateRateDto createRateDto = new CreateRateDto();
                createRateDto.StartDate = new DateOnly(
                                   Int32.Parse(item.RezDate.Split('|')[0].Split(".")[2]),
                                   Int32.Parse(item.RezDate.Split('|')[0].Split(".")[1]),
                                   Int32.Parse(item.RezDate.Split('|')[0].Split(".")[0]));
                createRateDto.EndDate = new DateOnly(
                                   Int32.Parse(item.RezDate.Split('|')[1].Split(".")[2]),
                                   Int32.Parse(item.RezDate.Split('|')[1].Split(".")[1]),
                                   Int32.Parse(item.RezDate.Split('|')[1].Split(".")[0]));
                createRateDto.RoomType = item.RoomType;
                createRateDto.B2B = float.Parse(item.B2B, System.Globalization.CultureInfo.InvariantCulture);
                createRateDto.WebBeds = float.Parse(item.WebBeds, System.Globalization.CultureInfo.InvariantCulture);
                createRateDto.OurBestPrice = float.Parse(item.OurBestPrice, System.Globalization.CultureInfo.InvariantCulture);
                createRateDto.OtherToPrice = float.Parse(item.OtherToPrice, System.Globalization.CultureInfo.InvariantCulture);
                createRateDto.CreatedDate = DateTime.UtcNow;
                createRateDto.HotelId = hotel.Id;
                await rateRepository.CreateRateAsync(createRateDto);
